...
export default {
    ...
    if(finalHits > 0){
            ...
            this.results = {
                totalHits:(Math.round(totalHits*100)/100),
                criticals: (Math.round(criticals*100)/100),
                finalHits: (Math.round(finalHits*100)/100),
                wounds: (Math.round(wounds*100)/100),
                totalWounds: (Math.round(totalWounds*100/100)),
                saves: (Math.round(saves*100)/100),
                damage: (Math.round(dmg*100)/100),
                warded:(Math.round(warded*100)/100)
            };
        },
        validateInputs() {
    if (this.attacks < 1) {
